...
public class Statement extends Node {
    ...
    public void expand() {
        if(this.getNodeType() == "IFStmt"){
            ...
            whileStmtExpand();
            this.setNodeType("WHILEStmt count("+((int) (Math.random()*3+1))+")");
        }
    }

    public void assignStmtExpand(){

        this.setOperator(new Leaf(this.selectBinaryOperatorExpression(), getDepth()+1, this, this.getNodeTool()));
        ...
        if(!(this.getNodeTool().getCurrentNodeNumber()>this.getNodeTool().getMaxNodes()))
            if(this.getNodeTool().addNewStmtDecision()){
                ...
                this.nextNode = new Statement(statementNodeTypeSelection, this.getDepth(), this, this.getNodeTool());
    ...
    public void whileStmtExpand(){
        ...
        this.getNodeTool().addNodeNumber();
        this.setOffspring(this.getOffspring()+1);
        this.setLeftNode(new Statement(statementNodeTypeSelection, this.getDepth()+1, this, this.getNodeTool()));
        ...
        this.getNodeTool().addNodeNumber();
        this.setConditionNode(new ConditionExpression(middleNodeTypeSelection, this.getDepth()+1, this, true, this.getNodeTool()));
        ...
        if(!(this.getNodeTool().getCurrentNodeNumber()>this.getNodeTool().getMaxNodes()))
            if(this.getNodeTool().addNewStmtDecision()){
                ...
                this.getNodeTool().addNodeNumber();
                this.setNextNode(new Statement(statementNodeTypeSelection, this.getDepth(), this, this.getNodeTool()));
    ...
    public void ifStmtExpand(){
        ...
        String middleNodeTypeSelection = this.getNodeTool().selectConditionExpression();
        this.getNodeTool().addNodeNumber();
        this.setConditionNode(new ConditionExpression(middleNodeTypeSelection, this.getDepth()+1, this, false, this.getNodeTool()));
        this.getConditionNode().expand();
        //this.setOffspring(this.getOffspring()+this.getConditionNode().getOffspring()+1);
        ...
        if(this.getNodeTool().addElseStmtDecision()){
            ...
            this.getNodeTool().addNodeNumber();
            this.setRightNode(new Statement(statementNodeTypeSelection, this.getDepth()+1, this, this.getNodeTool()));
            this.getRightNode().expand();
            //this.setOffspring(this.getOffspring()+this.getRightNode().getOffspring()+1);
        }
        if(!(this.getNodeTool().getCurrentNodeNumber()>this.getNodeTool().getMaxNodes()))
    ...
    public void draw(Pane canvas, int xStart, int yStart, int xEnd, int yEnd, int[] silhouette) {
        ...
        if(this.getOperator() != null) this.getOperator().draw(canvas, xEnd, yEnd, xEnd, yEnd+100, silhouette);
        if(this.getConditionNode() != null) this.getConditionNode().draw(canvas, xEnd, yEnd, xEnd, yEnd+100, silhouette);
        ...
        if(this.getNextNode() != null) this.getNextNode().draw(canvas, xEnd, yEnd, xEnd+100, yEnd, silhouette);
        ...
        if(this.getNodeType() == "IFStmt"){
            ...
            if (this.getLeftNode() != null) {
                while (this.getConditionNode().eval() == 0.0 && cont > 0) {
                    if (this.getLeftNode() != null)
                        res = this.getLeftNode().getPrefix(res) + " " +res;
                    cont--;
                }
            }
            if (nextNode != null)
    ...
    public Node getNode(int nodeNumber) {
        ...
        Node auxNode = null;
        boolean found = false;
        if(this.getNodeType().substring(0,3) == "WHI")
            this.getNodeTool().setIsWhileStmt(true);
        else
            this.getNodeTool().setIsWhileStmt(false);
